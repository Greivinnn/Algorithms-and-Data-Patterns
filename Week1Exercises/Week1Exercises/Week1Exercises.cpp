...
int main()
{
    ...
    // ask fredo for ecerything

    std::vector<int>& dice = listOfDice[d];
    for (int i = 0; i < dice.size(); ++i)
    {
        int value = dice[i];
        for (int d = 0; d < listOfDice.size(); ++d)
        {
            std::vector<int>& dice2 = listOfDice[d];
            for (int i2 = 0; i2 < dice2.size(); ++i2)
            {
                std::cout << "Pair: (" << value << ", " << dice2[i2] << ")\n";
            }
        }
    }
