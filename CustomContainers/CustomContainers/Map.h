...
public:
	...
	{
		ObtainKeysFromNode(mRootNode, outKeys);
	}
...
private:
	...
	void ObtainKeysFromNode(Node* node, Vector<KeyType>& outKeys) const
	{
		if(node != nullptr)
		{
			ObtainKeysFromNode(node->left, outKeys);
			outKeys.PushBack(node->kvp.key);
			ObtainKeysFromNode(node->right, outKeys);
		}
	}
